...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /start command. If called with 'register' arg, begin registration."""
    session_service: SessionService = context.bot_data["session_service"]
    user = update.effective_user
    args = context.args
    
    # Handle /start register (from deep link)
    if args and args[0] == "register":
        # Check if already registered
        existing_user = await session_service.get_user_by_telegram_id(user.id)
        if existing_user:
            await update.message.reply_text(
                f"✅ You're already registered as {existing_user['display_name']}!\n\n"
                "Use /login to start a session, or /status to check your status."
            )
            return
        
        # Store pending registration flag
        context.user_data["pending_register"] = True
        
        await update.message.reply_text(
            f"👋 Hi {user.first_name}!\n\n"
            f"Register for StockTracker Financial AI Assistant?\n\n"
            f"Reply **Yes** or **No**",
            parse_mode="Markdown"
        )
        return
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        await update.message.reply_text(
            ...
            "Click the **Register for Telegram Bot** button on our website to get started!",
            parse_mode="Markdown"
        )
        return
    
    # Create session
    await session_service.create_session(user["id"], telegram_user_id, telegram_chat_id)
