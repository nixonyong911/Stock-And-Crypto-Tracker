...
namespace SimFin.Worker.Controllers;

/// <summary>
/// API endpoints for managing SimFin fundamentals fetching.
/// </summary>
[ApiController]
[Route("api/fetch")]
...
{
    private const string DataSourceName = "SimFin";
    ...
    public async Task<ActionResult<TriggerResponse>> TriggerSingle(string symbol)
    {
        ...
        return Ok(new TriggerResponse
        {
            ...
            Success = success,
            Message = success ? "Fundamentals fetched successfully from SimFin" : "Failed to fetch fundamentals"
        });
    }
