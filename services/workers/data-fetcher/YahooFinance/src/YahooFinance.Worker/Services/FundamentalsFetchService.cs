...
public class FundamentalsFetchService : IFundamentalsFetchService
{
    ...
    {
        _logger.LogInformation("Starting fundamentals fetch for all active tickers");
    ...
    public async Task<bool> FetchFundamentalsForTickerAsync(StockTicker ticker, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogDebug("Fetching fundamentals for {Symbol} (ID: {Id})", ticker.Symbol, ticker.Id);

            // Fetch fundamentals
            var fundamentals = await _yahooClient.GetFundamentalsAsync(ticker.Symbol, ticker.Id, cancellationToken);
            ...
            var earnings = await _yahooClient.GetEarningsCalendarAsync(ticker.Symbol, ticker.Id, cancellationToken);
            foreach (var earning in earnings)
            {
                await _fundamentalsRepository.UpsertEarningsAsync(earning);
                _logger.LogDebug("Upserted earnings for {Symbol} on {Date}", ticker.Symbol, earning.EarningsDate);
            }
