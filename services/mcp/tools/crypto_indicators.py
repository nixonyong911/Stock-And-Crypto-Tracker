"""Crypto technical indicator tools with signal detection (crossovers, RSI zones)."""
...
async def get_crypto_technical_signals(
    conn,
    symbol: str,
    start_date: str,
    end_date: str,
    ...
    """
    Get daily technical indicators for a crypto asset with built-in signal detection.
    ...
    Uses UTC dates (crypto trades 24/7, no market-hours timezone conversion).
    ...
    query = """
        WITH daily AS (
            SELECT DISTINCT ON ((i.indicator_time AT TIME ZONE 'UTC')::date)
                ct.symbol,
                (i.indicator_time AT TIME ZONE 'UTC')::date AS indicator_date,
                i.sma, i.ema,
            ...
            ORDER BY (i.indicator_time AT TIME ZONE 'UTC')::date,
