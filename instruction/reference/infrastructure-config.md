...
| **Gateway 2.0 Code** | `/opt/stocktracker/repo/services/ai/gateway-2.0/` |
...
| **Gateway 2.0** | 8080 | HTTP | Docker internal only - `gateway-2.0:8080` |
...
| **Gateway 2.0 Health** | http://gateway-2.0:8080/health/live | Health check (Docker network) |
...
|----------|--------|---------|
| `AI_HUB_URL` | Docker Compose | `http://gateway-2.0:8080` |
| `AI_HUB_API_KEY` | Infisical | Authentication for Gateway 2.0 |
...
### Gateway 2.0 Docker Container

| Property | Value |
|----------|-------|
| **Container Name** | `gateway-2.0` |
| **Status Command** | `docker ps --filter name=gateway-2.0` |
| **Restart Command** | `docker restart gateway-2.0` |
| **Logs Command** | `docker logs gateway-2.0 -f` |
...
# Gateway 2.0 (Docker)
ssh -i "$HOME\.ssh\nx-linux-server-azure_key (1).pem" azureuser@20.17.176.1 "docker logs gateway-2.0 --tail 50"
```
...
# Gateway 2.0
ssh -i "$HOME\.ssh\nx-linux-server-azure_key (1).pem" azureuser@20.17.176.1 "docker restart gateway-2.0"
```
